...
void SHA256Hasher_99_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void SHA256Hasher_99_run(uint ctx_index,Circom_CalcWit* ctx);
...
uint get_total_signal_no() {return 204793;}

uint get_number_of_components() {return 1811;}

uint get_size_of_input_hashmap() {return 256;}

uint get_size_of_witness() {return 28906;}
...
for (uint i = 0; i < 1; i++) {
std::string new_cmp_name = "sha256hasher";
...
Fr_lt(&expaux[0],&lvar[1],&circuitConstants[30]); // line circom 35
while(Fr_isTrue(&expaux[0])){
{
uint cmp_index_ref = 0;
{
...
Fr_lt(&expaux[0],&lvar[1],&circuitConstants[136]); // line circom 40
while(Fr_isTrue(&expaux[0])){
{
PFrElement aux_dest = &signalValues[mySignalStart + ((1 * Fr_toInt(&lvar[1])) + 0)];
// load src
...
Fr_eq(&expaux[0],&signalValues[mySignalStart + ((1 * Fr_toInt(&lvar[1])) + 264)],&ctx->signalValues[ctx->componentMemory[mySubcomponents[0]].signalStart + ((1 * Fr_toInt(&lvar[1])) + 0)]); // line circom 42
...
SHA256Hasher_99_create(1,0,ctx,"main",0);
SHA256Hasher_99_run(0,ctx);
}
